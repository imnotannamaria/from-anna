...
- **`position: sticky` dies silently if any ancestor has `overflow` other than `visible`.** No error, no warning — the photograph just scrolls away. The reading view clips the hero with `overflow: clip`, which does not create a scroll container, and `overflow-x: hidden` is nowhere near `html` or `body`.
- **Decoration never paints over the letter.** The opening photograph drifts as the page scrolls, and at 96px it slid over the first line of the transcription. The drift is capped (`HERO_DRIFT`, mirrored as `--hero-drift`), the hero reserves that much room below it, and it clips on both axes so nothing it draws can reach the sheets.
